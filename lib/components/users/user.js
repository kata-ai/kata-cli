...
class User extends merapi_1.Component {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (options.token) {
                    ...
                    const result = yield this.helper.toPromise(this.api.authApi, this.api.authApi.cliLoginPost, { username: user, password: pass });
            ...
            try {
                ...
                if (type === "team") {
                    ...
                    const teams = response && response.body ?
                        response.body.teams.filter((team) => team.username === name) : [];
                    if (teams.length > 0) {
                        const team = teams[0];
                        console.log({ team });
                        const currentToken = this.helper.getProp("token") || {};
                        const currentLogin = this.helper.getProp("current_login") || "user";
                        this.setToken({ name, type: "team" }, currentToken[currentLogin]);
    ...
    impersonate(userName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const email = (yield this.getUserInfo(userName)).email;
                if (name && email) {
    ...
    getUserTokenInfo(name) {
        const tokenProp = (this.helper.getProp("token") || {});
        if (name in tokenProp) {
            const userName = name.toString();
            const token = tokenProp[name].toString();
            return {
                userName,
                token
            };
    ...
    getUserInfo(userName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { response } = yield this.helper.toPromise(this.api.userApi, this.api.userApi.usersGetInfoKeyGet, userName);
