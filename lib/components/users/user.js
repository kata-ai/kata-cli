    ...
    switch(type, name) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (type === "team") {
                    const info = yield this.helper.toPromise(this.api.userApi, this.api.userApi.usersUserIdGet, firstLogin.username);
                    const { data } = yield this.helper.toPromise(this.api.userApi, this.api.userApi.usersUserIdGet, name);
                    const teams = info ? info.data.teams.map((x) => x.teamId) : [];
                    if (data && teams.indexOf(data.id) > -1) {
                        const result = yield this.helper.toPromise(this.api.authApi, this.api.authApi.tokensPost, { type: "team", teamId: data.id });
                        const token = result.data.id;
