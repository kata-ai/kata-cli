        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                while (true) {
                    ...
                    const { response: { body } } = yield this.helper.toPromise(this.api.deploymentApi, this.api.deploymentApi.projectsProjectIdDeploymentVersionsGet, projectId, { limit: pageLimit, page });
                    if (body) {
                        const deployments = body.data;
                        const choicesDeployments = deployments.map((deployment) => ({ name: deployment.version, value: { version: deployment.version, id: deployment.id } }));
                        ...
                        let { changelog, confirm } = yield inquirer.prompt([
                            {
                                type: "text",
                                name: "changelog",
                                message: "Changelog:",
                        ...
                        if (confirm) {
                            const { response } = yield this.helper.toPromise(this.api.deploymentApi, this.api.deploymentApi.deploymentsDeploymentIdRollbackPost, version.id, requestBody);
