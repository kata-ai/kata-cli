...
class Channel {
    constructor(helper, api, config, environment) {
        this.helper = helper;
        this.api = api;
        this.config = config;
        this.environment = environment;
    }
    addChannel(channelName, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const projectId = this.helper.getProjectId();
            const environmentId = yield this.environment.askEnvironmentId();
            try {
                const { response: { body: channelsBody } } = yield this.helper.toPromise(this.api.deploymentApi, this.api.deploymentApi.projectsProjectIdEnvironmentsEnvironmentIdChannelsGet, projectId, environmentId, {});
                const channels = channelsBody;
    ...
    removeChannel(channelName, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const projectId = this.helper.getProjectId();
            const environmentId = yield this.environment.askEnvironmentId();
            try {
    ...
    inquireChannelData(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (additionalOptions) {
                options = Object.assign(Object.assign({}, options), additionalOptions);
            }
            ...
            return Object.assign(Object.assign({}, res), answer);
        });
    }
    updateChannel(channelName, options) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const projectId = this.helper.getProjectId();
                if (!projectId) {
                ...
                channelData.name = channelName;
                channelData = yield this.inquireChannelData(channelData);
                const result = yield this.helper.toPromise(this.api.deploymentApi, this.api.deploymentApi.projectsProjectIdEnvironmentsEnvironmentIdChannelsChannelIdPut, projectId, environmentId, channelFound.id, channelData);
                console.log(result.response.body);
                const channel = result.response.body;
                console.log("Channel added successfully");
                console.log(`Paste this url to ${channelData.type} webhook : ${channel.webhook}`);
