                            ...
                            for (const key in localDiff) {
                                if (entities.data[key]) {
                                    ...
                                    if (!nluDesc.entities[key].inherit) {
                                        yield this.helper.toPromise(this.api.nluApi, this.api.nluApi.projectsProjectIdNlusEntitiesEntityNamePut, nluDesc.name, key, Object.assign({}, nluDesc.entities[key], { name: key }));
                                    }
                                }
                                else {
                                    // Create new entity
                        ...
                        if (remoteDiff) {
                            for (const key in remoteDiff) {
                                if (!nluDesc.entities[key]) {
                                    ...
                                    yield this.helper.toPromise(this.api.nluApi, this.api.nluApi.projectsProjectIdNlusEntitiesEntityNameDelete, nluDesc.name, key);
                                }
                            }
                        }
                    }
